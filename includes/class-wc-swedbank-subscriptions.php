...
use WC_Gateway_Swedbank_Pay_Cc;

class WC_Swedbank_Subscriptions {
	const PAYMENT_ID = 'payex_psp_cc';
		...
		$payment_meta[ self::PAYMENT_ID ] = array(
			'swedbankpay_meta' => array(
				'token_id' => array(
					...
					'label' => __( 'Card Token ID', 'swedbank-pay-woocommerce-payments' ),
				),
			),
		);
	...
	public static function payment_meta_input( $subscription, $field_id, $field_value, $meta_data ) {
		...
		echo '<option value="">' . __( 'Please select', 'swedbank-pay-woocommerce-payments' ) . '</option>';
	 ...
	 *
	 * @return WC_Gateway_Swedbank_Pay_Cc
	 * @throws Exception
	 */
