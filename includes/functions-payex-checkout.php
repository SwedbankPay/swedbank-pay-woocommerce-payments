...
function px_capture_payment( $order, $amount = FALSE ) {
	...
	if ( ! $gateway ) {
		throw new \Exception( __( 'Unable to get payment instance.', 'woocommerce-gateway-payex-checkout' ) );
	}

	if ( ! method_exists( $gateway, 'capture_payment' ) ) {
		throw new \Exception( sprintf( __( 'Capture failure: %s', 'woocommerce-gateway-payex-checkout' ), 'Payment method don\'t support this feature' ) );
	}
...
function px_cancel_payment( $order ) {
	...
	if ( ! method_exists( $gateway, 'cancel_payment' ) ) {
		throw new \Exception( sprintf( __( 'Cancel failure: %s', 'woocommerce-gateway-payex-checkout' ), 'Payment method don\'t support this feature' ) );
	}
...
function px_refund_payment( $order, $amount = FALSE, $reason = '' ) {
	...
	if ( ! method_exists( $gateway, 'refund_payment' ) ) {
		throw new \Exception( sprintf( __( 'Refund failure: %s', 'woocommerce-gateway-payex-checkout' ), 'Payment method don\'t support this feature' ) );
	}

	if ( ! $gateway->can_refund( $order ) ) {
		throw new \Exception( __( 'Refund action is not available.', 'woocommerce-gateway-payex-checkout' ) );
	}
