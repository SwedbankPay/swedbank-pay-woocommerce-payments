...
// @see https://wiki.payex.com/xwiki/wiki/external/view/ecommerce/PayEx%20Checkout/FAQ/
...
class WC_Gateway_Payex_Checkout extends WC_Payment_Gateway_Payex
	...
	public function payment_scripts() {
		...
		if ( is_checkout() || isset( $_GET['pay_for_order'] ) || is_add_payment_method_page() ) {
			wp_register_script( 'wc-gateway-payex-checkout', untrailingslashit( plugins_url( '/', __FILE__ ) ) . '/../assets/js/payex-checkout.js', array(
				'jquery',
				'wc-checkout',
				'payex-checkout'
			), FALSE, TRUE );
		...
		if ( is_cart() || is_product() ) {
			wp_register_script( 'wc-gateway-payex-cart', untrailingslashit( plugins_url( '/', __FILE__ ) ) . '/../assets/js/payex-checkout-cart.js', array( 'payex-checkout' ), NULL, TRUE );
	...
	public function return_handler() {
		...
		switch ( $action ) {
			case 'pxcheckout':
				...
				// @todo Check compatibility with WC 2.6
...
// Register Gateway
WC_Payex_Checkout::register_gateway( 'WC_Gateway_Payex_Checkout' );
