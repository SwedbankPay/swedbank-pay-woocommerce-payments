...
class WC_Gateway_Swedbank_Pay_Vipps extends WC_Gateway_Swedbank_Pay_Cc {
	...
	public function __construct() {
		...
		$this->method_title = __( 'Vipps', 'swedbank-pay-woocommerce-payments' );
		$this->icon         = apply_filters(
			'wc_swedbank_pay_vipps_icon',
			plugins_url( '/assets/images/vipps.png', dirname( __FILE__ ) )
		);
		$this->supports     = array(
			'products',
			'refunds',
		...
		add_filter( 'swedbank_pay_vipps_phone_format', array( $this, 'vipps_phone_format' ), 10, 2 );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'title'          => array(
				...
				'default'     => __( 'Vipps payment', 'swedbank-pay-woocommerce-payments' ),
			),
			'description'    => array(
	...
	public function payment_scripts() {
		...
		wp_register_script(
			'wc-sb-vipps',
			untrailingslashit( plugins_url( '/', __FILE__ ) ) . '/../assets/js/seamless-vipps' . $suffix . '.js',
			array(
				'wc-sb-seamless',
			),
			false,
	...
	public function validate_fields() {
		...
		$billing_phone = apply_filters( 'swedbank_pay_vipps_phone_format', $billing_phone, null );

		$matches = array();
		preg_match( '/^(\+47)(?:4[015-8]|5[89]|87|9\d)\d{6}$/u', $billing_phone, $matches );
		if ( ! isset( $matches[0] ) || $matches[0] !== $billing_phone ) {
			wc_add_notice( __( 'Input your number like this +47xxxxxxxxx', 'swedbank-pay-woocommerce-payments' ), 'error' );
