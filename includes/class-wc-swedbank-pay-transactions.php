...
CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}payex_transactions` (
  ...
  KEY `order_id` (`order_id`)
		...
		if ( false === $result ) {
			throw new Exception(
				__( 'Failed to install the transaction table.', 'swedbank-pay-woocommerce-payments' )
			);
		}
	}
	...
	public function add( $fields ) {
		...
		return new WP_Error(
			'insert_failed',
			__( 'Failed to insert the transaction to the table.', 'swedbank-pay-woocommerce-payments' )
		);
	}
	...
	public function delete( $transaction_id ) {
		...
		if ( false === $result ) {
			return new WP_Error(
				'delete_failed',
				__( 'Failed to delete the transaction from the table.', 'swedbank-pay-woocommerce-payments' )
			);
		}
