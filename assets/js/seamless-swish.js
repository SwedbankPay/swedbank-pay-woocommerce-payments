    ...
    $( document ).ajaxComplete( function ( event, xhr, settings ) {
        if ( ( settings.url === wc_checkout_params.checkout_url ) || ( settings.url.indexOf( 'wc-ajax=complete_order' ) > -1 ) ) {
            ...
            try {
                ...
                if ( ! result.hasOwnProperty( 'is_swedbank_pay_swish' ) ) {
                    return false;
                }

                // Save js_url value
                wc_sb_swish.setJsUrl( result.js_url );
            } catch ( e ) {
                return false;
            }
        }
    ...
    window.wc_sb_swish = {
        ...
        init: function( form ) {
            ...
            $( this.form )
                ...
                .on( 'click', '#place_order', {'obj': window.wc_sb_swish}, this.onSubmit )

                // WooCommerce lets us return a false on checkout_place_order_{gateway} to keep the form from submitting
                .on( 'submit checkout_place_order_payex_psp_swish' );
        },
        ...
        initPaymentMenu: function ( id, callback ) {
            ...
            // Load Swish frame
            this.paymentMenu = window.payex.hostedView.swish( {
                container: id,
